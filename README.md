...
**Important Note:** The application runs automatically in the cloud on Railway, utilizing a fully managed PostgreSQL database endpoint. All operational and financial data is securely backed up and stored persistently. At the frontend streaming layer, a 24H Global Downcasting engine forces 64-bit floats and Python objects into 32-bit categories, safeguarding the 500MB RAM roof. The backend `etl_worker.py` utilizes asynchronous generator chunking (`chunksize=50000`) attached to regex vectorization engines to flatten ETL OOM loops. Heavy reports (Cohort Matrices, Player Master Lists, Financial Exports) are dispatched to a background `ReportQueue` thread so the menu stays responsive at all times.
