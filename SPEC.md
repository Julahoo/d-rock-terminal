...
- **Sidebar Router:** Role-based radio router (`view_mode`). Role access matrix:
  | Section | Superadmin | Admin | Operations | Financial |
  |---|---|---|---|---|
  | 📊 Dashboard | ✅ | ✅ | ✅ | ✅ |
  | 📞 Operations | ✅ | ✅ | ✅ | ❌ |
  | 🏦 Financial | ✅ | ✅ | ❌ | ✅ |
...
- **Password Security:** All passwords stored as SHA-256 hex digest via `hashlib.sha256(password.encode()).hexdigest()`. Login compares hashes, never plaintext.
- **Validation:** Username required, minimum 4-character password on create. Edit mode allows blank password (keeps existing hash).
...
- **Defensive Cold-Start Caching (The Inspector Guard):** To protect the database from generating massive `ERROR: relation does not exist` statements during cold-deployments (when background ETLs have not yet generated `cache_*` tables), the UI uses `inspect(engine).has_table(...)` from SQLAlchemy. If a dynamically generated ETL cache table is absent, the UI short-circuits to an empty dataframe BEFORE executing `pd.read_sql`, ensuring pristine logging and resilient app boots.
